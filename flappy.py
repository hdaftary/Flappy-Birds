...
import detect
from constant import get_speech
from opencamera import cap
...
def mainGame(movementInfo):
    ...
    while True:
        ...
        if len(sys.argv) >= 2 and sys.argv[1] == "True":
            flapped = detect.detect_eyes(frame)
        else:
            flapped = detect.detect_mouth(frame)
...
def showGameOverScreen(crashInfo):
    ...
    while True:
        ...
        if len(sys.argv) == 3 and sys.argv[2] == "False":
            speech = get_speech()
            if speech and "return" in speech:
                return
